...
class MapEnv(MultiAgentEnv):
    ...
    def step(self, actions):
        ...
        for agent in self.agents.values():
            ...
            apples_consumed += agent.compute_apples_consumed()
            beams_fired += agent.compute_beams_fired()
            times_hit += agent.compute_times_hit()
            tiles_cleaned += agent.compute_tiles_cleaned()
        
        infos['apples_consumed'] = apples_consumed
        infos['beams_fired'] = beams_fired
        infos['times_hit'] = times_hit
        infos['tiles_cleaned'] = tiles_cleaned
        ...
        if self.play_altruistic_game: # altruistic game stuff (also implement for just a single agent)
            ...
            infos['social_welfare'] = social_welfare
